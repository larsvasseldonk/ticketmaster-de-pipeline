...
class GCSUploader:
    ...
    def _check_bucket_ownership(self):
        ...
        if self.bucket_name not in project_bucket_ids:
            logging.error(
                f"Bucket '{self.bucket_name}' exists but does not belong to your project."
            )
            sys.exit(1)
        else:
            logging.info(
                f"Bucket '{self.bucket_name}' exists and belongs to your project."
            )
    ...
    def _create_bucket(self):
        ...
        try:
            ...
            logging.info(
                f"Cannot create the bucket '{self.bucket_name}'. Ensure you have permission."
            )
            sys.exit(1)
    ...
    def upload_file(self, file_path: str, max_retries: int = 3):
        ...
        for attempt in range(max_retries):
            try:
                logging.info(
                    f"Uploading {file_path} to {self.bucket_name} (Attempt {attempt + 1})..."
                )
...
def upload_files(
    file_paths, bucket_name, credentials_file=None, max_retries=3, max_threads=5
):
