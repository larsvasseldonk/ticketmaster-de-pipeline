...
class BigQueryLoader:
    ...
    def load_data(
        ...
        logging.info(
            f"Starting data load from {source_uri} to BigQuery table {table_ref}..."
        )
        ...
        try:
            ...
            load_job.result()
            logging.info(
                f"Successfully loaded {load_job.output_rows} rows into {stg_table_ref}"
            )
...
def list_csv_files(bucket_name: str) -> list:
    ...
    file_paths = [
        ...
        for blob in blobs
        if (blob.name.endswith(".csv")) & (blob.name.startswith(datetime_today))
    ]
...
def load_to_bq():
    ...
    write_disposition = (
        "WRITE_TRUNCATE"  # Options: WRITE_APPEND, WRITE_TRUNCATE, WRITE_EMPTY
    )
    ...
    for source_uri in file_paths:
        try:
            ...
            loader.load_data(
                dataset_id, table_name, source_uri, schema, write_disposition
            )
